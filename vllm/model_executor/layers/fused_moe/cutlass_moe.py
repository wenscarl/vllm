...
def run_cutlass_moe_fp4(
    ...
    c2 = ops.cutlass_fp4_moe_mm(int_fp4, w2_fp4, int_blockscale, w2_blockscale,
                                ...
                                blockscale_offsets[:-1], output.dtype, device)
    del int_fp4, int_blockscale

    c2 = ops.shuffle_rows(c2, c_map)
    out = (c2.view(m, num_topk, k) * topk_weights.view(m, num_topk, 1).half()).sum(dim=1)
    output.copy_(out.to(dtype=output.dtype), non_blocking=True)
...
class CutlassExpertsFp4(mk.FusedMoEPermuteExpertsUnpermute):
    ...
    def apply(
        ...
        # All required args must be provided
        assert w1_blockscale is not None and w2_blockscale is not None
        assert w1_alphas is not None and w2_alphas is not None
        assert a1_gscale is not None and a2_gscale is not None
        assert topk_weights is not None
        assert m is not None and n is not None and k is not None and e is not None
...
def cutlass_moe_fp4(
    ...
    fn = mk.FusedMoEModularKernel(
        MoEPrepareAndFinalizeNoEP(
            quant_dtype=torch.uint8,  # FP4 packed in uint8
